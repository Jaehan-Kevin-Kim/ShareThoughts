...
router.get("/:userId", async (req, res, next) => {
  try {
    const fullUserWithoutPassword: User | null = await User.findByPk(
      req.params.userId,
      {
        attributes: { exclude: ["password"] },
        include: [
    ...
    if (fullUserWithoutPassword) {
      ...
      const data = {
        ...fullUserWithoutPassword.toJSON(),
        posts: fullUserWithoutPassword.posts.length,
        followings: fullUserWithoutPassword.followings.length,
        followers: fullUserWithoutPassword.followers.length,
